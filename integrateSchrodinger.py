...
class IntegrateSchrodinger2D:
    ...
    '''
    def __init__(self, x, y, psi0, V, k0 = None, hbar=1, m=1, t0=0.0, dt=0.01,G12 = 0.01559, G11 = 1.05472):
    ...
    def diff_RK(self, t, psi_xy):
        ...
        nabla_psi_k = -psi_k*(1/2*self.G11*self.kx**2 + 1/2*self.G11*self.ky**2 + self.G12*self.kx*self.ky)

        # c) Obtain the kinetic part in the x space using the inverse FFT
        nabla_psi_xy = -self.compute_psi_x_from_psi_k(nabla_psi_k)
    ...
    def evolve(self, t_increase, store_all = False, save_file=False, step=3, out_file = 'time_evolution.csv'):
        '''
        ...
        #xmin,xmax,ymin,ymax =self.x[0,0],self.x[0,-1],self.y[0,0],self.y[-1,0]
        #norm = calc_norm(self.psi_xy, xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax)
        #self.psi_xy *=norm 
        ...
        # 3. Calculate psi(x, t+dt) = psi(x, t-dt) - 2i/h*dt*H*psi(x,t) 
            ...
            if save_file:
               if i%int(N_steps/30)==0:
                    ...
                    if i%step==0:
                        self.psi_xy_all = np.concatenate((self.psi_xy_all, self.psi_xy.reshape(1,self.Ny, self.Nx)), axis=0)
                        #print("Integrating Schrodinger Eq. Step {}/{}".format(i,N_steps))
        # Increase final time
        self.t += t_increase        
    def join_files(self, name_file, steps=30, reduce=False):
        for i in range(steps):
            filename = name_file + str(i) + '.npy'
